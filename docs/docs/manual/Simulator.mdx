...
- Depth simulation.
- Hands simulation.
- An optional simulator hand glow. When `simulator.renderToRenderTexture` is
  enabled, visible simulator hands keep their normal appearance while an
  always-on-top white glow is composited around their silhouette. Configure it
  with `options.simulator.handGlow`.
