...
Use `selectStart` and `squeezeStart` to trigger pinch (select) and fist (squeeze) gestures, which emit standard WebXR events (`selectstart`/`squeezestart` and `selectend`/`squeezeend`). Use `release` to terminate any active select or squeeze gesture and return the hand to its relaxed state. Raw pinching/fist rotations only change the visual pose; they do not trigger selection or squeezing state changes.
...
const result = await embodied.step({
  control: {rightHand: {release: true}},
});
