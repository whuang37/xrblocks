 ...
 * @version v0.20.0
 * @commitid ab86a13
 * @builddate 2026-08-10T18:35:28.948Z
...
const defaultScriptMethods = new WeakSet();
...
function markDefaultScriptMethods(prototype) {
    for (const name of Object.getOwnPropertyNames(prototype)) {
        ...
        if (typeof method === 'function')
            defaultScriptMethods.add(method);
    }
}
...
function defineApiKeyPromptElement() {
    if (customElements.get(PROMPT_TAG))
        return;
...
class AI extends Script {
    ...
    async initializeModel(ModelClass, modelOptions) {
        ...
        if (!apiKey || !this.isValidApiKey(apiKey)) {
            ...
            if (isRunningInGeminiCanvas()) {
                ...
                console.warn(`No valid API key found for ${this.options.model}. ` +
    ...
    getUrlApiKey(modelOptions) {
        return (getUrlParameter(this.options.globalUrlParams.key) ||
...
const xrDeviceCameraEnvironmentOptions = deepFreeze$1(new DeviceCameraOptions({
    ...baseCaptureOptions,
    videoConstraints: {
...
const xrDeviceCameraUserOptions = deepFreeze$1(new DeviceCameraOptions({
    ...baseCaptureOptions,
    videoConstraints: {
        ...baseCaptureOptions.videoConstraints,
        facingMode: 'user',
...
const xrDeviceCameraEnvironmentContinuousOptions = deepFreeze$1(new DeviceCameraOptions({
    ...xrDeviceCameraEnvironmentOptions,
    willCaptureFrequently: true,
}));
const xrDeviceCameraUserContinuousOptions = deepFreeze$1(new DeviceCameraOptions({
...
class XRDeviceCamera extends VideoStream {
     ...
     */
    async initStream_() {
        if (this.isInitializing_ || this.disposed_)
            return;
    ...
    startXRCameraAccessFallback_(reason, error) {
        ...
        this.xrCameraAccessTimeout_ = setTimeout(() => {
            if (this.disposed_ || !this.useXRCameraAccess_ || this.loaded)
                return;
            this.useXRCameraAccess_ = false;
            this.setState_(StreamState.NO_DEVICES_FOUND, { force: true });
...
const xrDepthMeshOptions = deepFreeze$1(new DepthOptions({
    enabled: true,
    depthMesh: {
        enabled: true,
...
const xrDepthMeshVisualizationOptions = deepFreeze$1(new DepthOptions({
    enabled: true,
    depthMesh: {
        enabled: true,
...
const xrDepthMeshPhysicsOptions = deepFreeze$1(new DepthOptions({
    enabled: true,
    depthMesh: {
        enabled: true,
        updateVertexNormals: false,
 ...
 */
class DirectTouch {
    static { this.EXIT_PADDING = 0.01; }
    constructor(registry, resolver) {
        this.registry = registry;
        this.resolver = resolver;
        this.active = new Map();
        this.awaitingExit = new Set();
        this.contacts = [];
    ...
    update(inputs) {
        ...
        for (const input of inputs) {
            ...
            if (!previous && resolved?.target) {
                ...
                if (resolved)
                    this.awaitingExit.add(input.controller);
...
class GazeDwell {
    ...
    update(controller, resolved, deltaSeconds, paused = false) {
        ...
        const movement = (state.lastPoint?.distanceTo(point) ?? 0) /
        ...
        if (movement > MOVEMENT_THRESHOLD)
            ...
            state.elapsed = Math.min(DWELL_SECONDS, state.elapsed + delta);
...
class HitRegistry {
    ...
    setWorldTouchCandidates(objects) {
        ...
        for (const [physical, entry] of this.touchCandidates) {
            if (!this.registered.has(entry))
                this.touchCandidates.delete(physical);
        }
        for (const object of next) {
            if (this.touchCandidates.has(object))
                continue;
    ...
    raycast(scene, ray, intersections) {
        ...
        for (const intersection of intersections) {
            if (hasPrivateAncestor$1(intersection.object))
                continue;
        ...
        for (const { physical } of this.registered) {
            if (physical.xb?.pointerEvents === 'none')
                continue;
            if (this.isBelowScene(physical, scene) && !hasPrivateAncestor$1(physical)) {
                continue;
            }
            if (!effectiveVisible$1(physical))
                continue;
            if (!physical.layers.test(this.raycaster.layers))
                continue;
...
function intersectTree(object, raycaster, intersections) {
    ...
    if (object.userData.xrblocksPrivate === true)
        return;
    if (object.xb?.pointerEvents === 'none')
        return;
...
function compareTouchIntersections(a, b, preferred) {
    if (a.object === b.object)
        return 0;
    if (a.object === preferred)
        return -1;
    if (b.object === preferred)
        return 1;
    const aOverlay = isOverlayHit(a.object);
    const bOverlay = isOverlayHit(b.object);
    if (aOverlay !== bOverlay)
...
function isOverlayHit(object) {
    let current = object;
    while (current) {
        if (current.userData.xrblocksOverlay === true)
            return true;
        current = current.parent;
    }
    return false;
...
function hasPrivateAncestor$1(object) {
    let current = object;
    while (current) {
        if (current.userData.xrblocksPrivate === true)
            return true;
        current = current.parent;
    }
    return false;
}
function effectiveVisible$1(object) {
    let current = object;
    while (current) {
        if (!current.visible)
            return false;
...
class HitResolver {
    ...
    nearestTarget(path, callbackTarget, manipulationOwner) {
        if (!callbackTarget)
            return manipulationOwner;
        if (!manipulationOwner)
...
class InteractionSourceState {
    ...
    updateRay(input) {
        ...
        if (input.position)
            this.position.copy(input.position);
        else
            this.controller.getWorldPosition(this.position);
        if (input.orientation)
            this.orientation.copy(input.orientation);
        else
            this.controller.getWorldQuaternion(this.orientation);
...
function selectionInvalidReason(selection, ancestry, semanticDisabled) {
    ...
    for (let index = 0; index + 1 < ancestry.length; index++) {
        if (ancestry[index].parent !== ancestry[index + 1])
            return 'removed';
    }
...
function createPlanarSurfaceProjector(surface) {
    ...
    return (ray) => {
        ...
        if (!localRay.intersectPlane(plane, localPoint))
            return undefined;
        return {
...
class ReticlePresenter {
    ...
    present(snapshot, resolved) {
        ...
        const beyondPresentationRange = resolved &&
...
function suspendTransformScripts(owner) {
    for (const child of owner.children) {
        if (child instanceof TransformScript)
            child.suspend();
    }
}
...
const COLOR_KEYS = new Set(['color']);
const NONNEGATIVE_KEYS = new Set([
    'width',
    'height',
    'minWidth',
    'minHeight',
...
class UIElement extends Script {
    constructor(kind, options = {}) {
        ...
        if (isUIRootKind(kind))
            rootReferences.add(new WeakRef(this));
    ...
    set style(style) {
        ...
        for (const key of Object.keys(this.styleTarget)) {
...
function collectUIRoots(target) {
    ...
    for (const reference of rootReferences) {
        ...
        if (root)
            ...
            rootReferences.delete(reference);
    }
}
...
function createNestedStyleValue(property, value, onChange) {
    ...
    if (isStateStyleKey(property)) {
        return createStyleProxy(cloneStyleValue(value), true, onChange);
    }
    if (property === 'transform') {
        return createTransformProxy(cloneStyleValue(value), onChange);
    }
    return value;
}
...
function validateStyle(property, value, stateOnly) {
    ...
    if (value === undefined)
        return;
    if (stateOnly && !STATE_STYLE_KEYS.has(property)) {
    ...
    if (NUMBER_KEYS.has(property) &&
        (typeof value !== 'number' || !Number.isFinite(value))) {
        throw new Error(`UI style "${property}" must be a finite number.`);
    }
    if (PAINT_KEYS.has(property) && !isPaint(value)) {
        throw new Error(`UI style "${property}" must be a valid paint.`);
    }
    if ((property === 'innerShadowPosition' || property === 'dropShadowPosition') &&
        !isVector2Like(value)) {
    ...
    if (property === 'lineHeight' && !isLineHeight(value)) {
        throw new Error('UI style "lineHeight" must be a nonnegative number, pixel value, or percentage.');
    }
    if (POSITION_KEYS.has(property) && !isUIPosition(value)) {
        throw new Error(`UI style "${property}" must be a finite number or percentage.`);
    }
    ...
    if ((property === 'width' || property === 'height') && !isUIUnit(value)) {
        throw new Error(`UI style "${property}" must be a finite number, percentage, or "auto".`);
    }
    if (NONNEGATIVE_KEYS.has(property) &&
        typeof value === 'number' &&
        value < 0) {
    ...
    if ((property === 'innerShadowFalloff' || property === 'dropShadowFalloff') &&
        typeof value === 'number' &&
        value <= 0) {
...
function validateTransformValue(property, value) {
    if (value !== undefined && !isUIPosition(value)) {
        throw new Error(`UI transform "${property}" must be a finite number or percentage.`);
    }
}
function isPaint(value) {
    ...
    if (!['linear', 'radial', 'angular', 'diamond'].includes(gradient.gradientType ?? '') ||
...
function isVector2Like(value) {
    if (value === undefined || value instanceof THREE.Vector2)
        return true;
    return (Array.isArray(value) &&
        value.length === 2 &&
        value.every((component) => typeof component === 'number' ? Number.isFinite(component) : false));
}
...
function normalizeRotationAxis(axis) {
    const vector = axis === undefined || axis === 'y'
...
function faceCameraQuaternion(worldPosition, cameraPosition, parentWorldQuaternion, mode = 'cylindrical', result = new THREE.Quaternion(), scratch) {
    ...
    if (!parentWorldQuaternion)
        return result.copy(worldQuaternion);
...
class RotateDriver {
    ...
    capture(session) {
        ...
        if (!axis || !Number.isFinite(sensitivity))
            return undefined;
        return {
            action: this.action,
...
class ScaleDriver {
    ...
    propose(session, baseline) {
        if (!session.auxiliary)
            return undefined;
        const currentDistance = session.primary.snapshot.position.distanceTo(session.auxiliary.position);
...
class ManipulationManager {
    ...
    resolve(path) {
        ...
        for (const current of path) {
            ...
            if (!hasChildHandle && options?.manipulationHandle !== undefined) {
                hasChildHandle = true;
                handle = current;
                const action = options.manipulationHandle === 'none'
                    ? ManipulationAction.None
                    : options.manipulationHandle.action;
                if (action !== undefined && !isHandleAction(action))
                    return undefined;
                childHandle = action;
            }
            if (options?.manipulation !== undefined) {
                if (options.manipulation === false ||
                ...
                if (requested === ManipulationAction.None)
                    return undefined;
    ...
    tryStart(capture, snapshot) {
        ...
        const session = {
            ...
            primaryAction: resolution.action === ManipulationAction.Scale
                ? undefined
                : resolution.action,
        };
        ...
        const baseline = session.primaryAction
            ...
            : undefined;
        if (session.primaryAction && !baseline)
            return false;
    ...
    tryClaimScale(snapshot) {
        ...
        for (const session of this.sessions.values()) {
            if (!session.config.scale || session.auxiliary)
                continue;
            if (eligible)
                return false;
            eligible = session;
        ...
        if (baseline?.action !== ManipulationAction.Scale)
            return false;
        try {
            if (session.phase)
    ...
    updateSession(session) {
        ...
        if (!session.phase.defaultPrevented)
            proposal.apply();
        try {
    ...
    finishAuxiliary(session, source, phase) {
        ...
        if (!session.primaryAction)
            return;
        try {
            if (this.startPhase(session, session.primaryAction))
                return;
        }
        catch (error) {
            this.removeSession(session, true);
    ...
    releaseAuxiliaryRole(session, source) {
        if (this.roles.get(source) === session)
    ...
    dispatchPhase(session, active, phase, proposal) {
        ...
        if (phase === 'start')
            active.defaultPrevented = preventState.value;
    }
}
function getCardEdge(owner) {
    const candidate = owner;
    if (!isUIElement(owner) ||
...
class Interaction {
    constructor(dependencies) {
        ...
        this.frameSnapshots = [];
        this.rawIntersections = new Map();
        this.resolvedRays = new Map();
        this.hoverPaths = new Map();
        this.captures = new Map();
        this.exclusiveControls = new Map();
    ...
    setLongSelectDuration(seconds) {
        if (!Number.isFinite(seconds) || seconds < 0) {
            throw new Error('Options.interaction.longSelectDuration must be finite and nonnegative.');
        }
    ...
    update(frame, deltaSeconds = 0) {
        ...
        for (const input of frame.raySources)
            nextSources.add(input.controller);
        for (const input of frame.directTouches)
            nextSources.add(input.controller);
        ...
        for (const [controller, capture] of this.captures) {
            if (capture.kind === 'target') {
                const reason = selectionInvalidReason(capture.selection, capture.ancestry, capture.semanticControl !== undefined &&
    ...
    cancelObject(object, reason = 'removed') {
        ...
        for (const [controller, touch] of this.touches) {
            if (!selectionBelongsTo(touch.selection, object))
                continue;
            const contact = this.directTouch.remove(controller);
            if (contact)
                this.processTouchContact(contact);
        }
    ...
    updateRay(input, deltaSeconds, deliberate) {
        ...
        if (input.sourceType === 'gaze') {
            const semantic = resolved?.semanticControl
                ...
                : undefined;
            const gazeTarget = semantic?.kind === 'button' && resolved?.semanticControl
                ? resolved
                : undefined;
    ...
    collectIntersections(input, previousSelected) {
        ...
        const shouldRaycast = this.raycastMode === 'continuous' ||
    ...
    beginSelection(controller, gaze = false) {
        ...
        if (this.suppressedUntilRelease.has(controller))
            return;
        if (claimedScale) {
    ...
    endSelection(controller, reason, releasedTarget, finalSnapshot) {
        ...
        if (completed)
            this.callbacks.invokeGlobal('onSelect', globalEvent);
    ...
    updateTouch(contact, snapshot) {
        if (contact.phase === 'start' && contact.resolved?.target) {
            const selection = this.createSelection(contact.controller, contact.resolved);
            const touchState = {
                selection,
            ...
            try {
                ...
                if (!prevented) {
                    this.startTargetCapture(contact.controller, snapshot, contact.resolved, true);
                }
        ...
        try {
            ...
            if (!touch.prevented) {
                ...
                if (contact.endReason === 'left-target' &&
                    capture?.kind === 'target' &&
                    capture.touch &&
                    capture.action !== 'none') {
                    this.endSelection(contact.controller, 'released', touch.selection.target, snapshot);
                }
                else {
                    this.cancelCapture(contact.controller, contact.endReason === 'source-lost'
                        ? 'source-lost'
                        : 'released-outside');
                }
            }
    ...
    updateGrab(touch, contact, snapshot) {
        ...
        if (!touch.grabbing) {
            ...
            dispatchInteractionPath(this.callbacks, touch.selection.scriptPath, 'onObjectGrabbing', event);
        }
    }
    ...
    updateSemantic(capture, snapshot) {
        ...
        if (resolved?.surface === capture.selection.surface &&
    ...
    updateLongSelect(capture, snapshot, deltaSeconds) {
        ...
        if (capture.longSelectDuration < this.longSelectDuration)
            return;
    ...
    setResolvedRay(controller, snapshot, resolved) {
        ...
        if (resolved)
            this.resolvedRays.set(controller, resolved);
        else
    ...
    updateHoverPath(controller, resolved, previous) {
        ...
        if (nextPath.length > 0)
            this.hoverPaths.set(controller, nextPath);
        else
            this.hoverPaths.delete(controller);
        ...
        const eventFor = (value) => ({
            source: this.getSourceSnapshot(controller)?.source ??
    ...
    createSelection(controller, resolved) {
        ...
        return {
            source: controller,
            publicSource: this.getSourceSnapshot(controller)?.source ??
                getInteractionSource(controller, 'controller-ray'),
            target,
    ...
    createSelectEvent(controller, capture) {
        ...
        const intersection = resolved && surface && objectIsDescendantOf(resolved.surface, surface)
            ? clonePublicIntersection(resolved.intersection, resolved.surface)
            : undefined;
        return {
            source: targetCapture?.selection.publicSource ??
    ...
    hasDeliberateInput(frame) {
        if (frame.raySources.some((input) => input.sourceType !== 'gaze' && input.selected) ||
            this.touches.size > 0) {
            return true;
        }
        for (const capture of this.captures.values()) {
...
function isDescendantOf$2(object, ancestor) {
    let current = object;
    while (current) {
        if (current === ancestor) {
            return true;
...
function isInternalRoot(object) {
    return (object.userData.xrblocksPrivate === true ||
...
function inferName(object) {
    const semanticObject = object;
    return (semanticObject.ariaLabel ??
...
function inferValue(object) {
    if (!isUIElement(object) || getUIElementKind(object) !== 'slider')
        return {};
    const slider = object;
...
function createSemanticNode(object, id, semantic, parentId) {
    ...
    if (semantic.min !== undefined)
        node.min = semantic.min;
    if (semantic.max !== undefined)
        node.max = semantic.max;
...
function isObjectInLineOfSight({ camera, object, targetPoint, raycastTargets, occlusionOpacityThreshold, }) {
    ...
    const occludingHit = hits.find((hit) => {
        ...
        if (isDescendantOf$2(hit.object, object) ||
            isDescendantOf$2(object, hit.object)) {
            return false;
        }
...
class ScriptsManager extends THREE.EventDispatcher {
    constructor(initScriptFunction) {
        ...
        this.interactionCandidates = new Set();
        ...
        this.hasTargetHandler = (object, sourceType) => {
            if (!this.isScript(object))
                return false;
            const script = object;
            const hooks = sourceType === 'direct-touch'
        ...
        this.invokeManipulation = (script, event) => {
            if (!this.hasOverriddenHook(script, 'onObjectManipulate'))
                return;
    ...
    initScript(script) {
        ...
        if (this.activeScripts.has(script))
            return SCRIPT_READY;
        if (this.failedScripts.has(script))
            return SCRIPT_READY;
    ...
    uninitScript(script) {
        ...
        if (!this.activeScripts.delete(script))
            return;
    ...
    syncScriptsWithScene(scene) {
        ...
        for (const script of this.pendingInitializations.keys()) {
            ...
            const pending = this.pendingInitializations.get(script);
            if (pending)
                this.syncPromises.push(pending.promise);
            this.uninitScript(script);
        }
    ...
    callHook(hook, callback) {
        ...
        if (scripts)
            this.callTargeted(scripts, hook, callback);
    }
    ...
    unindexScript(script) {
        for (const scripts of this.hookScripts.values())
...
function eventForTarget(argument, currentTarget) {
    if (!argument || typeof argument !== 'object')
        return argument;
    if (Reflect.get(argument, 'currentTarget') === currentTarget)
        return argument;
    const event = Object.create(Object.getPrototypeOf(argument));
    Object.defineProperties(event, Object.getOwnPropertyDescriptors(argument));
...
class XRButton {
    ...
    dispose() {
        if (this.disposed)
            return;
        this.disposed = true;
        this.sessionManager.removeEventListener(WebXRSessionEventType.UNSUPPORTED, this.onUnsupported);
...
class Input {
    constructor() {
        ...
        this.releasedControllers = new Set();
    ...
    bindKeyDown(event) {
        if (this.keyDownListeners.has(event))
            return;
        this.keyDownListeners.add(event);
        window.addEventListener('keydown', event);
    }
    bindKeyUp(event) {
        if (this.keyUpListeners.has(event))
            return;
    ...
    getRaySourceType(controller) {
        if (controller === this.mouseController)
            return 'mouse';
        if (controller === this.gazeController)
            return 'gaze';
...
class User extends Script {
    ...
    getIntersectionAt(obj, id = -1) {
        return this.interaction.getIntersectionAt(obj, id < 0 ? undefined : this.controllers[id]);
    }
    /**
...
const UI_THEME_STYLE_ROLES = new Set([
    'surface',
    'panel',
    'text',
    'button',
...
function createThemeSnapshot(value) {
    ...
    const styles = value.styles === undefined ? undefined : cloneThemeStyles(value.styles);
...
function updateThemeSnapshot(theme, update) {
    ...
    const colors = update.colors === undefined
        ? theme.colors
        : { ...theme.colors, ...cloneColors(update.colors, true) };
    return createThemeSnapshot({
        colors,
        borderRadius: update.borderRadius === undefined
            ? theme.borderRadius
            : update.borderRadius,
...
function cloneThemeStyles(value) {
    ...
    for (const [kind, style] of Object.entries(value)) {
        if (!UI_THEME_STYLE_ROLES.has(kind)) {
...
class UI {
    ...
    set theme(value) {
        ...
        if (!snapshot)
            throw new Error(`Unknown UI theme preset "${value}".`);
...
const WARNED_OVERLAY_TRANSFORMS = new WeakSet();
...
class UIRenderer {
    constructor(interaction, loader = defaultLoader, reportError) {
        ...
        this.validate = (root) => {
            ...
            const records = root
                ? [this.mounts.get(root)].filter((record) => record?.connected === true)
            ...
            for (const record of records)
                issues.push(...record.mount.validate());
    ...
    reconcile(deltaSeconds, camera) {
        if (!this.publicScene || !this.renderer)
            return;
        const roots = this.collectConnectedRoots();
        ...
        for (const root of this.mounts.keys()) {
            if (!this.connectedRoots.has(root))
        ...
        if (this.backendState.kind === 'idle') {
            ...
            void this.loadBackend().catch((error) => {
                ...
                if (this.reportError) {
                    for (const root of loadingRoots)
                        this.reportError(error, root);
                }
                else {
        ...
        for (let order = 0; order < roots.length; order++) {
            const root = roots[order];
            if (!this.mounts.has(root))
                this.mount(root, backend, order);
        }
    ...
    collectConnectedRoots() {
        ...
        for (const root of this.roots) {
            if (isDescendantOf$1(root, scene)) {
...
function placeObjectAtIntersectionFacingTarget(obj, intersection, target) {
    ...
    matrix4$1.makeBasis(rightVector, worldNormal, forwardVector);
...
function getObjectBoundingBox(object) {
    ...
    function traverse(node) {
        if (!node.visible || node.userData.xrblocksPrivate === true)
            return;
...
class Core {
    ...
    constructor(simulatorLoader = loadSimulatorModule) {
        ...
        this.uiRenderer = new UIRenderer(this.interaction, undefined, (error, root) => this.scriptsManager.reportError(error, root, 'UI renderer load'));
        ...
        this.scriptsManager.afterDispose = (script) => {
            if (isUIElement(script))
                this.uiRenderer.release(script);
        };
    ...
    async disposeWebXRSessionManager() {
        ...
        manager.removeEventListener(WebXRSessionEventType.SESSION_START, this.onWebXRSessionStarted);
        manager.removeEventListener(WebXRSessionEventType.SESSION_END, this.onXRSessionEnded);
        manager.removeEventListener(WebXRSessionEventType.UNSUPPORTED, this.onWebXRUnsupported);
    ...
    init(options = new Options()) {
        ...
        if (this.lifecycleState === 'disposing' ||
            this.lifecycleState === 'disposed') {
            return Promise.reject(new Error(`Core cannot initialize after disposal has ${this.lifecycleState === 'disposing' ? 'started' : 'completed'}.`));
        }
    ...
    async initialize(options) {
        ...
        this.webXRSessionManager.addEventListener(WebXRSessionEventType.SESSION_START, this.onWebXRSessionStarted);
        ...
        this.webXRSessionManager.addEventListener(WebXRSessionEventType.UNSUPPORTED, this.onWebXRUnsupported);
        ...
        if (this.physics) {
            this.physicsInterval = setInterval(this.physicsStep, 1000 * this.physics.timestep);
        }
...
class FaceCamera extends TransformScript {
    ...
    update() {
        ...
        if (!this.canUpdate || !object || !this.camera || !this.timer)
            return;
...
class FollowHead extends TransformScript {
    static { this.dependencies = { camera: THREE.Camera, timer: THREE.Timer }; }
    constructor(options) {
        super();
...
class FollowObject extends TransformScript {
    constructor(options) {
        super();
 ...
 */
class Orbit extends TransformScript {
    static { this.dependencies = { camera: THREE.Camera, timer: THREE.Timer }; }
    constructor(options) {
        super();
        this.meanAnomaly = 0;
        ...
        this.worldQuaternion = new THREE.Quaternion();
        if (!options?.target)
            throw new Error('Orbit requires a target object.');
        this.target = options.target;
        ...
        const eccentricity = options.eccentricity ?? (path === 'elliptical' ? 0.2 : 0);
        ...
        this.frame = oneOf(options.frame ?? 'world', ['world', 'target', 'view'], 'frame');
    ...
    init({ camera, timer }) {
        ...
        if (!owner)
            return;
        if (isAncestorOrSelf(owner, this.target) ||
            isAncestorOrSelf(this.target, owner)) {
            throw new Error('Orbit target must not be its owner, ancestor, or descendant.');
        }
        this.semiMajorRadius = this.configuredRadius;
    ...
    update() {
        const owner = this.parent;
        if (!this.canUpdate || !owner || !this.timer)
            return;
        ...
        if (this.precessionPeriod !== undefined) {
            this.precessionAngle = normalizeAngle(this.precessionAngle + (TAU * delta) / this.precessionPeriod);
        }
        this.updateOrbitOrientation();
    ...
    applyClearance(owner) {
        const minimumPeriapsis = worldBoundingRadius(this.target) +
...
function solveKepler(meanAnomaly, eccentricity) {
    ...
    let lower = -Math.PI;
    let upper = Math.PI;
    let eccentricAnomaly = eccentricity < 0.8 || mean === 0 ? mean : Math.sign(mean) * Math.PI;
    for (let iteration = 0; iteration < 16; iteration += 1) {
        const error = eccentricAnomaly - eccentricity * Math.sin(eccentricAnomaly) - mean;
        if (Math.abs(error) <= 1e-12)
            return eccentricAnomaly;
        if (error > 0)
            upper = eccentricAnomaly;
        else
            lower = eccentricAnomaly;
...
function positive(value, name) {
    finite(value, name);
    if (value <= 0)
        throw new Error(`Orbit ${name} must be greater than zero.`);
    return value;
}
...
function nonnegative(value, name) {
    finite(value, name);
    if (value < 0)
        throw new Error(`Orbit ${name} must be nonnegative.`);
    return value;
}
...
class VisibilityTransition extends TransformScript {
    ...
    init({ timer }) {
        ...
        if (this.targetVisible)
            this.visibleScale.copy(this.parent.scale);
    }
    show() {
        if (!this.parent)
            return;
    ...
    update() {
        ...
        this.progress = THREE.MathUtils.clamp(this.progress + direction * (delta / Math.max(this.duration, 0.001)), 0, 1);
...
class UIButton extends UIElement {
    constructor({ label, icon, ariaLabel, disabled = false, onClick, children, ...options } = {}) {
        ...
        if (children?.length && (label !== undefined || icon !== undefined)) {
            throw new Error('UIButton convenience label/icon content cannot be combined with children.');
        }
        if (!ariaLabel && !label) {
...
class UIIcon extends UIElement {
    constructor({ icon, variant = 'outlined', weight = 400, filled = false, ariaLabel, ...options }) {
        if (!icon)
            throw new Error('UIIcon requires an icon name.');
    ...
    set icon(value) {
        if (!value)
            throw new Error('UIIcon.icon must be a non-empty string.');
        if (value === this._icon)
            return;
        this._icon = value;
        this.markUIDirty();
...
class UIText extends UIElement {
    constructor({ text, ...options }) {
        if (typeof text !== 'string')
            throw new Error('UIText requires text.');
        super('text', options);
        this.name = 'UIText';
        this._text = text;
    }
...
class UISlider extends UIElement {
    constructor({ ariaLabel, min = 0, max = 1, step = 0.01, value = 0, disabled = false, onInput, onChange, ...options }) {
        if (!ariaLabel)
            throw new Error('UISlider requires ariaLabel.');
        if (typeof disabled !== 'boolean') {
    ...
    updateInput(input) {
        const ratio = clamp$1(input.uv?.x ?? 0.5, 0, 1);
...
function validateRange(min, max, step) {
    if (!Number.isFinite(min) ||
        ...
        min > max ||
        step <= 0) {
        throw new Error('UISlider requires finite values with min <= max and step > 0.');
    }
}
...
function quantize(value, min, max, step) {
    ...
    return clamp$1(Number(stepped.toPrecision(12)), min, max);
}
...
class RotationHitSurface extends THREE.Mesh {
    constructor(bounds) {
        ...
        geometry.scale(Math.min(halfX, maxRadius) + padding, Math.max(size.y, 0.001), Math.min(halfZ, maxRadius) + padding);
        super(geometry, new THREE.MeshBasicMaterial({ colorWrite: false, depthWrite: false }));
...
class ModelViewer extends Script {
    ...
    constructor({ origin = 'bottom-center', manipulation, platformMargin = PLATFORM_MARGIN, autoplay = true, occlusion = false, castShadow = true, receiveShadow = true, } = {}) {
        ...
        this.occludableMaterials = new WeakSet();
        this.occludableShaders = new Set();
    ...
    onHoverExit(event) {
        ...
        if (this.hoveringControllers.size === 0)
            this.platform?.setHovered(false);
    }
    dispose() {
    ...
    clearHitRegistrations() {
        for (const unregister of this.unregisterHitSurfaces)
            unregister();
...
function alignOrigin(root, bounds, origin) {
    if (origin === 'source' || bounds.isEmpty())
        return;
