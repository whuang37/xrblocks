...
export class TransformGizmo extends xb.Script {
  ...
  private setRay(controller: THREE.Object3D): void {
  ...
  override onSelectEnd(event: SelectEvent) {
    if (event.source.controller !== xb.core.input.mouseController) return;
    this.endDrag();
  }
  ...
  updateTranslateDrag(drag: TranslateDrag) {
    ...
    if (!this.raycaster.ray.intersectPlane(plane, currentPoint)) return;
