...
describe('createVisibleObjectsContext', () => {
  it('ignores pointer-events-disabled objects by default', () => {
    ...
    expect(context.nodes.target.view?.inLineOfSight).toBe(true);
  });

  it('can include pointer-events-disabled objects in occlusion checks', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera(60, 1, 0.1, 10);
    const target = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.2, 0.2));
    const occluderRoot = new THREE.Group();
