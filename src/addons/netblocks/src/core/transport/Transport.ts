...
export abstract class Transport extends EventTarget {
  ...
  protected emitPeerJoin(peerId: string): void {
    this.dispatchEvent(
      new CustomEvent<TransportPeerEventDetail>('peer-join', {detail: {peerId}})
    );
  }

  protected emitPeerLeave(peerId: string): void {
    this.dispatchEvent(
      new CustomEvent<TransportPeerEventDetail>('peer-leave', {
        detail: {peerId},
      })
    );
  }
  ...
  protected emitMessage(peerId: string, data: TransportPayload): void {
    this.dispatchEvent(
      new CustomEvent<TransportMessageEventDetail>('message', {
        detail: {peerId, data},
      })
    );
  }
