...
For a pre-configured template using TypeScript and Vite, check out <CodeSearchLink path="templates/typescript/src/main.ts"/> (and its associated [package.json](file:///Users/davili/Documents/xrblocks/templates/typescript/package.json) and [tsconfig.json](file:///Users/davili/Documents/xrblocks/templates/typescript/tsconfig.json)).
...
```bash
npm install @dimforge/rapier3d @google/genai openai lit @pmndrs/uikit @preact/signals-core @mediapipe/tasks-vision @mediapipe/tasks-audio troika-three-text @sparkjsdev/spark three-mesh-bvh temporal-polyfill
```
...
- **Performance & Utilities**: `three-mesh-bvh` (Accelerated raycasting), `temporal-polyfill` (Timing standard)
